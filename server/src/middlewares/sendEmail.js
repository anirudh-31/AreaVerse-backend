import { getPassowordResetEmail, getPasswordResetAlertEmail, getVerificationEmail } from "../utils/createEmail.js";
...
/**
 * Function to send a password reset alert email to the user.
 * @param {object} options
 * @param {string} options.to Recipient email.
 * @param {string} options.userName User's name.
 * @param {string} options.resetLink password reset URL.
 */
async function sendPasswordResetAlert({ to, userName, resetLink }){
    const companyName     = process.env.APP;
    const logoUrl         = path.join(path.resolve(path.dirname('')), 'src', 'assets', 'logo.png');
    const supportEmail    = 'support@example.com';
    const unsubscribeLink = 'https://example.com/unsubscribe';

    const htmlContent = getPasswordResetAlertEmail({
        user_name         : userName,
        company_name      : companyName,
        logo_url          : logoUrl,
        reset_link        : resetLink,
